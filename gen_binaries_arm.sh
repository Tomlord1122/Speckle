...
CONFIG=arm_tom_after
CONFIGFILE=${CONFIG}.cfg
RUN="/opt/qemu/bin/qemu-aarch64 -cpu cortex-a57 -d plugin -plugin /home/tomlord/workspace/Speckle/insn.so -plugin /home/tomlord/workspace/Speckle/libhotblocks.so -D"
...
if [ "$compileFlag" = true ]; then
   ...
   cd $SPEC_DIR; . ./shrc; time runspec --config ${CONFIG} --size ${INPUT_TYPE} --action setup perlbench bzip2 gcc mcf gobmk hmmer sjeng libquantum h264ref omnetpp astar milc namd soplex  lbm sphinx3
#   cd $SPEC_DIR; . ./shrc; time runspec --config ${CONFIG} --size ${INPUT_TYPE} --action scrub int
   ...
   for b in ${BENCHMARKS[@]}; do
      ...
      BMK_DIR=$SPEC_DIR/benchspec/CPU2006/$b/run/run_base_${INPUT_TYPE}_arm.0000;
      
      echo ""
      echo "ls $SPEC_DIR/benchspec/CPU2006/$b/run"
      ls $SPEC_DIR/benchspec/CPU2006/$b/run
      ls $SPEC_DIR/benchspec/CPU2006/$b/run/run_base_${INPUT_TYPE}_arm.0000
      echo ""
      ...
      if [ "$copyFlag" = true ]; then
         ...
         cp $BUILD_DIR/../run_arm.sh $COPY_DIR/run_arm.sh
         sed -i '4s/.*/INPUT_TYPE='${INPUT_TYPE}' #this line was auto-generated from gen_binaries.sh/' $COPY_DIR/run_arm.sh
         for f in $BMK_DIR/*; do
            echo $f
            if [[ -d $f ]]; then
         ...
         mv $COPY_DIR/$b/${SHORT_EXE}_base.arm $COPY_DIR/$b/${SHORT_EXE}
      fi
   done
fi
