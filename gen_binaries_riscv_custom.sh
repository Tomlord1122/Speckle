...
RUN="/opt/qemu-custom/bin/qemu-riscv64 -L /opt/riscv/sysroot -cpu rv64,v=true,zba=true,zbb=true,zbs=true,vlen=128,vext_spec=v1.0 -d plugin -plugin /home/tomlord/workspace/Speckle/insn.so -plugin /home/tomlord/workspace/Speckle/libhotblocks.so -D"
CMD_FILE=commands.txt
INPUT_TYPE=ref

# the integer set
# BENCHMARKS=(400.perlbench 453.povray)
...
if [ "$compileFlag" = true ]; then
   ...
   cd $SPEC_DIR; . ./shrc; time runspec --config ${CONFIG} --size ${INPUT_TYPE} --action setup gcc bzip2 perlbench
   # cd $SPEC_DIR; . ./shrc; time runspec --config ${CONFIG} --size ${INPUT_TYPE} --action setup perlbench bzip2 gcc mcf gobmk hmmer sjeng h264ref omnetpp astar povray soplex
#   cd $SPEC_DIR; . ./shrc; time runspec --config ${CONFIG} --size ${INPUT_TYPE} --action scrub int
   ...
   for b in ${BENCHMARKS[@]}; do
      ...
      BMK_DIR=$SPEC_DIR/benchspec/CPU2006/$b/run/run_base_${INPUT_TYPE}_riscv.0000;
      
      echo ""
      echo "ls $SPEC_DIR/benchspec/CPU2006/$b/run"
      ls $SPEC_DIR/benchspec/CPU2006/$b/run
      ls $SPEC_DIR/benchspec/CPU2006/$b/run/run_base_${INPUT_TYPE}_riscv.0000
      echo ""
      ...
      if [ "$copyFlag" = true ]; then
         ...
         cp $BUILD_DIR/../run_riscv.sh $COPY_DIR/run_riscv.sh
         sed -i '4s/.*/INPUT_TYPE='${INPUT_TYPE}' #this line was auto-generated from gen_binaries.sh/' $COPY_DIR/run_riscv.sh
         for f in $BMK_DIR/*; do
            echo $f
            if [[ -d $f ]]; then
         ...
         mv $COPY_DIR/$b/${SHORT_EXE}_base.riscv $COPY_DIR/$b/${SHORT_EXE}
      fi
   done
fi
   ...
   for b in ${BENCHMARKS[@]}; do
      ...
      for input in "${commands[@]}"; do
         if [[ ${input:0:1} != '#' ]]; then # allow us to comment out lines in the cmd files
            echo "~~~Running ${b}"
            echo "  ${RUN} ${b}_${count}.log ${SHORT_EXE}_base.${CONFIG} ${input}"
            eval ${RUN} ${b}_${count}.log ${SHORT_EXE}_base.${CONFIG} ${input}
         fi
      done
